package com.bandit.util

object ConstantsTest {
    const val adminEmail = "Admin@Bandit.com"
    const val adminPassword = "adminbandit"
    const val adminNickname = "Admin"

    const val newUserEmail = "android@test.com"
    const val newUserPassword = "androidtest"
    const val accountName = "Android Test"
    const val accountNewName = "Best Tester"
    const val accountNewNickname = "Bester"
    const val accountNickname = "Android-er"
    const val accountRole = "Manager"
    const val bandName = "Admin's Band"

    const val noBandEmail = "no@band.com"
    const val noBandPassword = "nobandnoband"

    const val maximumDelayLoadingScreen = 7_500L //ms
    const val maximumDelayOperations = 2_000L //ms
    const val smallDelay = 1_000L //ms
    const val fabAnimationDelay = 300L //ms
}